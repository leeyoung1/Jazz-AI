...
这条命令会把 JazzAI 的受管资产安装到当前仓库的 `.claude/` 目录下，并完成这些动作：

- 复制 skills 到 `.claude/skills/`、agents 到 `.claude/agents/`（直接成为 Claude Code 斜杠命令与子代理）
- 复制 hooks 运行时到 `.claude/jazzai/hooks/`
- 把受管 hooks 写入 `.claude/settings.json` 的 `hooks` 字段（只增删受管 router 条目，保留你的自定义 hooks）
- 写入 `.claude/jazzai/install.json` 作为受管安装标记（含本次安装的 skills/agents 清单）
- 写入 `.claude/jazzai/VERSION` 作为仓库内运行时版本标记
- 创建 `.claude/jazzai/runtime/` 运行时目录
- 在项目模式下补齐 `.gitignore` 中与 runtime 相关的忽略规则

目标仓库中的工作流目录遵循 Claude Code 约定：
...
- `jazz-ai update`：把目标仓库里的 `.claude` 资产同步到当前 CLI 版本
- `jazz-ai uninstall`：默认只清理受管 `.claude` 资产
...
# 再把当前仓库里的 .claude 资产更新到这个 CLI 版本
jazz-ai update
```
...
- `jazz-ai update`：更新当前仓库里的 `.claude` 资产，不会升级 CLI 包本身

### 0.5 全局模式与项目模式

- `--mode project`：默认模式。安装到目标仓库下的 `.claude/`
- `--mode global`：安装到 `~/.claude/`，供多个仓库复用同一套资产

### 0.6 卸载语义

- `jazz-ai uninstall`：删除受管 `.claude` 资产（受管 skills/agents、`.claude/jazzai/hooks`、settings.json 中的受管 hooks），默认保留 `.prd/.vibe/.archive` 及你的自有资产
