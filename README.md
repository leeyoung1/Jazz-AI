...
`jazz-ai install` 默认把受管资产安装到当前仓库的 `.claude/` 目录（skills/agents 直接落地为 Claude Code 斜杠命令与子代理，hooks、运行时与安装标记收纳在 `.claude/jazzai/` 隔离子目录）；`--mode global` 可安装到 `~/.claude/` 供多个仓库复用。`--yes` 适用于无交互安装。

安装会把受管 hooks 写入 `.claude/settings.json` 的 `hooks` 字段（全局模式写 `~/.claude/settings.json`），只增删 JazzAI 受管的 router 条目，保留你已有的 skills、agents 和自定义 hooks。

需要特别区分两层更新：

- `npm update -g jazzai`：更新全局 `jazz-ai` CLI 包
- `jazz-ai update`：把当前仓库已安装的 `.claude` 资产同步到当前 CLI 版本，不会升级 CLI 自身

卸载语义分为两层：

- `jazz-ai uninstall`：清理当前仓库内受管的 `.claude` 资产（按清单移除受管 skills/agents、删除 `.claude/jazzai/hooks`、从 settings.json 摘除受管 hooks），默认保留 `.prd/.vibe/.archive` 及你的自有资产
- `jazz-ai uninstall --all`：连项目产物一并清理
- `npm uninstall -g jazzai`：只卸载全局 CLI 包，不会改任何仓库

## 概述

JazzAI 是一套面向 AI 辅助开发的分阶段工作流，遵循 Claude Code 的 `.claude` 目录约定组织 skills、agents、hooks 和运行时。它把"需求 → 方案 → 拆解 → 编码 → 归档"沉淀为可恢复、可门控、可注入上下文的 slash 命令工作流，配合 Claude Code 的 hooks 能力按阶段自动注入产物上下文。

- 资产目录：`.claude/`（skills/agents 直接落地，JazzAI 运行时隔离在 `.claude/jazzai/`）
- hooks 注册：`.claude/settings.json`
...
```text
.claude/
├── skills/                 # 斜杠命令（/prd-input、/tech-design …）
├── agents/                 # TDD 子代理（tdd-planner/executor/verifier …）
├── settings.json           # hooks 注册（受管 router 条目 + 你的自定义）
└── jazzai/                 # JazzAI 隔离运行时
    ├── hooks/              # router.mjs + lib/
    ├── settings.template.json
    ├── install.json        # 受管安装标记
    ├── VERSION
    └── runtime/            # 会话状态（gitignore）
