...
import { handleHookPayload } from "../router.mjs";
...
test("expanded command prompt resolves back to workflow command", async () => {
  ...
  const response = await handleHookPayload({
    ...
    cwd: rootDir,
    prompt:
      "执行 JazzAI 工作流的阶段3a：技术方案生成或解析。\n\n使用 `/tech-design` skill 执行。\n\n参数: --project demo\n\n请先加载 `.codeflicker/skills/tech-design/SKILL.md`，然后按照其中的工作流执行。",
  });
...
test("session store migrates legacy session payloads", () => {
  const rootDir = createFixture();
  const sessionsDir = path.join(rootDir, ".codeflicker", "runtime", "sessions");
...
test("session-store last_outputs excludes removed internal artifacts", () => {
  const rootDir = createFixture();
  const state = saveSessionState(rootDir, { session_id: "outputs" });
  assert.ok("tech_design" in state.last_outputs);
  assert.ok("execution_report" in state.last_outputs);
  assert.equal("team_binding" in state.last_outputs, false);
  assert.equal("pipeline_state" in state.last_outputs, false);
  assert.equal("regression_report" in state.last_outputs, false);
});
